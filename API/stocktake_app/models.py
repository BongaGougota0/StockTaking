...
class StoreAdmin(db.Model):
    ...
    #Categories created
    cate = db.relationship('Category', backref='create_cat', lazy=True)
...
class Product(db.Model):
    ...
    creator = db.Column(db.Integer, db.ForeignKey('store.store_id'), nullable=False)

    #each product falls under a category

    def __repr__(self):
        return f"Product('{self.product_name}', '{self.product_price}','{self.product_category}', '{self.product_description}', '{self.img_product}')"
